...
These analyses are not yet implemented in code but are straightforward additions using the existing outputs (`city_bucket_docs.csv` and `metrics.json`).【F:src/features/build_city_month_docs.py†L60-L87】【F:src/models/train_baseline.py†L95-L152】

## Ethics and responsible use
This project is an academic prototype and explicitly does not claim operational predictive capability. It is built on news data with known coverage and framing biases, and the current label is synthetic. Outputs should not be used for policy, security, or humanitarian decision-making. Once ACLED labels are integrated, additional safeguards should include bias audits, transparency on geographic coverage, and careful communication about uncertainty and limitations. Any deployment should follow responsible research practices and include human oversight. 【F:README.md†L72-L88】【F:src/models/train_baseline.py†L95-L106】
